...
def build_narrative_context(
    ...
    # the schedule row substitutes the values via the shared
    # ``substitute_sign_description`` helper (#51 — the same call
    # audit.sign_table makes, so the two surfaces cannot drift) and
    # appends the host-sign suffix.  The station derivation mirrors the
    # layout + audit.sign_table.  W7-3a's value is station-independent
    # (deterministic V1 miles default inside the helper).
    if params.road_type == "freeway" and params.closure_type == "shoulder":
        ...
        _, w7_3a_value = substitute_sign_description(
            "W7-3a", sign_a_station, params, taper_start_station=taper_start_station
        )
        w16_2a_text = f"{w16_2a_value} (under upstream W21-5aR)"
        w7_3a_text = f"{w7_3a_value} (under downstream W21-5aR)"
