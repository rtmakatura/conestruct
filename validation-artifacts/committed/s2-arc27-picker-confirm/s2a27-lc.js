// s2-arc27 live check — #270 Confirm in the picker row's action cell; the
// s2-arc26 harness (#254 + #255) extended with leg C3.  Same legs, plus:
//   C3  every DETECTED row's picker opened in turn (only a detected row may
//       enter the reason state, spec 46): pickerH per condition — equal ×N
//       at 1440 (GO ruling 3), recorded at 380 — and Confirm right == the
//       action edge ±1 for the longest and shortest names; Cancel closes.
// (was) s2-arc26 live check — #254 + #255 the site-conditions block, at
...
(async () => {
  ...
  for (const vp of [{ width: 1440, height: 1000 }, { width: 380, height: 800 }]) {
    ...
    // C3 — every detected row's picker: height per condition, Confirm on the action edge
    await rowByName(page, detected[0].name).getByRole("button", { name: "Cancel" }).click(); await page.waitForTimeout(150);
    const pickers = [];
    for (const d of detected) {
      await rowByName(page, d.name).getByRole("button", { name: "Dismiss" }).click(); await page.waitForTimeout(200);
      const pc = await page.evaluate(CHIPS);
      const actionRight = await page.evaluate(() => { const b = document.querySelector("#site-corrections .site-correction-row .sc-action button"); return b ? Math.round(b.getBoundingClientRect().right) : null; });
      const confirmRight = pc.confirm.left + pc.confirm.w;
      const rowH = await page.evaluate(() => Math.round(document.querySelector("#site-corrections .site-correction-picker").getBoundingClientRect().height * 10) / 10);
      pickers.push({ name: d.name, nameLen: d.name.length, pickerH: pc.pickerH, rowH, confirmRight, actionRight, confirmTop: pc.confirm.top, lines: Math.round(pc.chips.length ? new Set(pc.chips.map((x) => x.h && Math.round(x.box[0]))).size : 0) });
      await rowByName(page, d.name).getByRole("button", { name: "Cancel" }).click(); await page.waitForTimeout(150);
    }
    fs.writeFileSync(path.join(OUT, `${tag}-C3-pickers.json`), JSON.stringify(pickers, null, 1));
    const hs = [...new Set(pickers.map((x) => x.rowH))];
    const edge = pickers.every((x) => Math.abs(x.confirmRight - x.actionRight) <= 1);
    const longest = pickers.reduce((a, b) => (b.nameLen > a.nameLen ? b : a)); const shortest = pickers.reduce((a, b) => (b.nameLen < a.nameLen ? b : a));
    check(tag, "C3 confirm edge", edge, pickers.map((x) => `"${x.name}" Confirm right ${x.confirmRight} vs action ${x.actionRight}`).join(" ; ") + ` | longest "${longest.name}" shortest "${shortest.name}"`);
    check(tag, "C3 picker heights", vp.width === 380 ? pickers.length > 0 : hs.length === 1, `${vp.width === 380 ? "recorded (not required equal at 380)" : "equal x" + pickers.length}: ` + pickers.map((x) => `"${x.name}" row ${x.rowH} (line ${x.pickerH})`).join(" ; ") + ` | ${await L.shot(page, OUT, `${tag}-C3-last`)}`);
    await rowByName(page, detected[0].name).getByRole("button", { name: "Dismiss" }).click(); await page.waitForTimeout(150);
    // S1 — stage two
