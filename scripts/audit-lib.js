...
// s2-audit-1/audit-lib.js, which stays as recorded (as does its twin in
// s2-triage-2).  Unchanged but for this note.  It opens no page itself —
// shaGate reads Modal /healthz, which is outside the gate — so the bypass
// header goes in its CALLERS, per C-Q7: every caller that opens a context
// runs `await require("./gate.cjs").applyGate(ctx, SITE)` on it (not
// extraHTTPHeaders, which would send the token to third parties).
//
